...
adam <- function(data, model="ZXZ", lags=c(frequency(data)), orders=list(ar=c(0),i=c(0),ma=c(0),select=FALSE),
    ...
    creator <- function(...){
        return(adam_creator(...,
                            ...
                            smoother=smoother, nonZeroARI=nonZeroARI, nonZeroMA=nonZeroMA));
    }
    filler <- function(...){
        return(adam_filler(...,
    ...
    CF <- function(B,
        ...
        penaltyValue <- adam_bounds_checker(adamElements, adamElements$arimaPolynomials,
                                            bounds,
    ...
    estimator <- function(etsModel, Etype, Ttype, Stype, lags, lagsModelSeasonal, lagsModelARIMA,
        ...
        if(regressors=="select"){
            # This is a failsafe for weird cases, when something went wrong with estimation
    ...
    #### Combination of models ####
        ...
        adamSelected$icWeights <- adam_ic_weights(icSelection);
...
print.adam <- function(x, digits=4, ...){
    ...
    etsModel <- etsChecker(x);
    arimaModel <- arimaChecker(x);
    cesModel <- cesChecker(x);
    sparmaModel <- sparmaChecker(x);
...
covarOPG <- function(object, stepSize=.Machine$double.eps^(1/4)){
    ...
    # v1 scope: pure ETS (no ARIMA, no xreg, no constant). Those need their own
    # initial-perturbation map, added later; until then the caller falls back to
    # the Hessian. Detect out-of-scope parameters by name and structure.
    etsParameterNames <- c(persistenceNames, "phi", "level", "trend");
    isSeasonal <- substr(parametersNames,1,8)=="seasonal";
    inScope <- (parametersNames %in% etsParameterNames) | isSeasonal;
    if(!all(inScope) || !is.null(initialList$arima) || !is.null(initialList$xreg)){
        return(NULL);
    }
    ...
    # Evaluate the per-observation log-likelihood at a perturbed parameter set,
    # rebuilt through the standard adam() path with everything provided (empty
    # B, single evaluation), preserving the selected ETS structure and lags.
    evalPointLik <- function(persistence, phi, initialArg){
        modelLocal <- try(suppressWarnings(
            adam(object$data, model=modelString, lags=modelLags,
    ...
    for(j in seq_len(nParam)){
        ...
        makeSet <- function(delta){
            persistence <- object$persistence;
            phi <- object$phi;
            initialArg <- initialList;
            if(nameJ %in% persistenceNames){
                ...
                # xreg / ARIMA / constant initials: not covered by this v1 map.
                return(NULL);
            }
            return(list(persistence=persistence, phi=phi, initialArg=initialArg));
        }

        setUp <- makeSet(hStep);
        setDown <- makeSet(-hStep);
        if(is.null(setUp) || is.null(setDown)){
            return(NULL);
        }
        likUp <- evalPointLik(setUp$persistence, setUp$phi, setUp$initialArg);
        likDown <- evalPointLik(setDown$persistence, setDown$phi, setDown$initialArg);
...
forecast.adam <- function(object, h=10, newdata=NULL, occurrence=NULL,
    ...
    # Get componentsNumberETS and componentsNumberARIMA
...
# data / states / matrices — no ts/zoo wrapping, no S3 class. The caller
# is responsible for the public-facing wrapping (time index, class
# assignment, extra fields).
# ----------------------------------------------------------------------
simulateADAMCore <- function(object, nsim=1, obs=nobs(object), ...){
    ellipsis <- list(...);
    ...
    if(is.null(object$occurrence) || is.character(object$occurrence)){
        # ADAM without intermittency OR om/omg (where ``$occurrence`` is
        # a scheme string like "odds-ratio", not a fitted model).
        # For om/omg the 0/1 occurrence draw happens in simulate.om /
        # simulate.omg using the latent-derived probability — here we
        # just feed the C++ simulator a no-op all-ones mask.
...
simulate.adam <- function(object, nsim=1, seed=NULL, obs=nobs(object), ...){
    ...
    if(any(yClasses=="zoo")){
        ...
        matrixYt <- zoo(array(inner$data,c(obsInSample,nsim),
                              dimnames=list(NULL,paste0("nsim",c(1:nsim)))),
                        order.by=yTime);
    }
    else{
        matrixYt <- ts(array(inner$data,c(obsInSample,nsim),
                             dimnames=list(NULL,paste0("nsim",c(1:nsim)))),
    ...
    return(structure(list(timeElapsed=Sys.time()-startTime, model=object$model, distribution=object$distribution,
                          data=matrixYt, states=inner$states, persistence=object$persistence,
                          measurement=inner$matWt, transition=object$transition, initial=object$initial,
                          probability=inner$pt, occurrence=object$occurrence,
                          residuals=inner$matErrors, other=inner$ellipsis),
