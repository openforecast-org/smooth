...
test_that("gradient falls back to backcasting for unsupported models", {
    set.seed(1)
    y <- abs(sim.es("MMN", obs = 80, persistence = c(0.3, 0.1),
                    initial = c(100, 1.01))$data)
    fB <- adam(y, "MMN", initial = "backcasting", silent = TRUE)
    fG <- suppressWarnings(adam(y, "MMN", initial = "gradient", silent = TRUE))
    # Multiplicative model is out of Phase-1 scope: identical to backcasting
